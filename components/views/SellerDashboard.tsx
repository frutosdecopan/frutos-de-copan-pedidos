...
import { Plus, Search, ShoppingCart, CheckCircle2, Clock, MapPin, Package, Edit2, ChevronDown, ChevronUp, MessageSquare, CalendarDays, Filter, XCircle } from 'lucide-react';
...
export const SellerDashboard = ({
    ...
    useEffect(() => {
        if (propEditingOrderId && orders.length > 0) {
            ...
            if (orderToEdit) {
                ...
                setClientRtn(orderToEdit.clientRtn || '');
                ...
                setSelectedOrderType(orderToEdit.orderType || 'Venta');
    ...
    if (citiesLoading || productsLoading || presentationsLoading || destinationsLoading || orderTypesLoading) {
        return (
            <div className="p-6">
    ...
    const resetForm = () => {
        ...
        setSelectedDestination(destinations[0]?.name || '');
        setSelectedOrderType(orderTypes[0]?.name || 'Venta');
    ...
    const handleEditOrder = (order: Order) => {
        ...
        setSelectedOrderType(order.orderType || orderTypes[0]?.name || 'Venta');
        setCart(order.items);
        setMode('create');
    };
                            ...
                            <div>
                                <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                                    <span className="flex items-center gap-1">
                                        ...
                                        Fecha Programada de Entrega <span className="text-gray-400">(opcional)</span>
                                    </span>
                                </label>
                                <input
                                    type="date"
    ...
    return (
        <div className="p-4 md:p-8 max-w-5xl mx-auto">
                    ...
                    displayOrders.map(order => (
                        <div key={order.id} className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-5 hover:shadow-md transition-shadow">
                                    ...
                                    <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 flex flex-col gap-1">
                                        ...
                                        {order.deliveryDate && (
                                            <span className="flex items-center text-orange-600 dark:text-orange-400 font-medium">
                                                ...
                                                Entrega programada: {new Date(order.deliveryDate + 'T12:00:00').toLocaleDateString('es-HN', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' })}
                                            </span>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="flex justify-end items-center border-t dark:border-gray-800 pt-3 gap-3">
                                ...
                                <button
                                    onClick={() => handleOpenComments(order)}
                                    className="text-blue-600 dark:text-blue-400 font-medium flex items-center bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/30 px-4 py-2 rounded-lg transition-colors shadow-sm border border-blue-200 dark:border-blue-800 relative"
                                >
                                ...
                                {/* Edit Button - Only for draft/sent orders */}
            ...
            {/* Comments Modal - Read Only for Sellers */}
            {showCommentsModal && selectedCommentsOrder && (
                <OrderCommentsReadOnlyModal
